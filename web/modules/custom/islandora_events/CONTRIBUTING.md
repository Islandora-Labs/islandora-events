...
Optionally, if you want to run a specific test suite such as `kernel`,
provide it via the `TEST_SUITE` environment variable. Leave it blank to run all
tests:
...
If you are running functional JavaScript tests, start a chromedriver container:
...
For the standard local path, use the module `Makefile`. This mirrors the module
GitHub Actions workflow:

```bash
make test DRUPAL_VERSION=$DRUPAL_VERSION PHP_VERSION=$PHP_VERSION
```
...
If you prefer to run the container command directly, this is what `make test`
uses:
...
docker run \
    ...
    --hostname drupal \
    --rm \
    --volume $(pwd):/var/www/drupal/web/modules/contrib/islandora_events:ro \
    --env ENABLE_MODULES="islandora islandora_events" \
...
- `make network-down` will clean up local test infrastructure.
