...
namespace Drupal\islandora_events_fcrepo\Commands;
...
/**
 * Fedora/fcrepo operator commands for Islandora Events.
 */
final class IslandoraEventsFcrepoCommands extends DrushCommands {
  ...
  public function __construct(
    ...
    private FcrepoIndexTarget $target,
    private LoggerInterface $logger,
  ) {}

  /**
   * Replays one stored Fedora/fcrepo ledger row.
   */
  #[CLI\Command(name: 'islandora-events-fcrepo:index-record')]
  #[CLI\Argument(name: 'event-record-id', description: 'Event record ID to replay through the embedded Fedora/fcrepo indexer.')]
  public function indexRecord(int $eventRecordId): int {
    ...
    if ((string) $record->get('target_system')->value !== 'fedora') {
      throw new \InvalidArgumentException(sprintf(
        'Event record %d is not a Fedora/fcrepo record.',
        $eventRecordId,
      ));
    }
    ...
    try {
      ...
      $this->logger->error('Fedora/fcrepo replay failed for record @id: @message', [
        '@id' => $eventRecordId,
